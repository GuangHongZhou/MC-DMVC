...
def valid(model, device, dataset, view, data_size, class_num, tracker):
    ...
    acc, nmi, purity, fscore, precision, recall, ari = evaluate(labels_vector, y_pred)

    flag = tracker.update(nmi, ari, acc, purity)
    return flag


def valid_vis(model, device, dataset, view, data_size, class_num):
    test_loader = DataLoader(
        dataset,
        batch_size=256,
        shuffle=False,
    ...
    print(f"ACC: {acc:.4f}, NMI: {nmi:.4f}, ARI: {ari:.4f}, "
          f"Purity: {purity:.4f}, F-score: {fscore:.4f}, "
